...
const CertificateNumber = async ({ searchParams }) => {
  ...
  if (!certificate) {
    return (
      <div className="card p-6 flex items-center justify-center min-h-[calc(100vh-83px)] relative">
        <p className="text-accent font-semibold sm:text-2xl text-4xl">
  ...
  return (
    <div className="flex min-h-[calc(100vh-83px)] justify-center p-2 pt-10">
      <div className="w-4/5">
        <FilledCertificate certificate={certificate} download />
      </div>
    </div>
  );
};
