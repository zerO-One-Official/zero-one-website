...
const OnGoingEvent = ({ events }) => {
  ...
  return onGoingEvents.length ? (
    <section
      ref={ref}
      className={`flex xl:flex-col flex-auto mt-40 sm:mt-20 fadeonscroll`}
    >
      <div className={`mt-0 sm:mt-10 pr-11 box-border w-2/5 xl:w-full`}>
        <h2 className={`sticky top-36 text-6xl sm:text-5xl font-semibold`}>
          Ongoing
          ...
          {upcomingEvents.map((event) => {
            ...
            return (
              <Link
                href={`/events/${event.name}?tab=info`}
                key={event._id}
                className="group bg-white/5 border border-white/10 hover:scale-105 transition-all rounded flex flex-1 justify-between items-center p-4 w-full gap-6 sm:gap-2 "
              >
...
const Past = ({ events }) => {
  ...
  return (
    <section
      ref={ref}
      className={`flex xl:flex-col flex-auto my-40 sm:my-20 fadeonscroll`}
    >
      <div className={`mt-0 sm:mt-10 pr-11 box-border w-2/5 xl:w-full`}>
        <h2 className={`sticky top-36 text-6xl sm:text-5xl font-semibold`}>
          Past
