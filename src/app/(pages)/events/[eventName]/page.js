...
const EventPage = ({ params }) => {
  ...
  const eventEndDate = new Date(
    eventStartDate.getTime() + data?.event?.duration * 60 * 60 * 1000
  );
    ...
    <div className="container-70 min-h-screen pt-10 flex flex-col">
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between mb-6 gap-2">
          <button
            onClick={() => {
              router.push(`${eventName}?tab=info`);
            }}
            className={`flex items-center justify-center gap-2 xs:p-2 p-4 flex-1 text-xl ${
              activeTab === "info"
                ? "bg-primary-light text-primary  fill-primary"
                : "fill-primary-light"
            } rounded-sm`}
            title="Informations"
          >
          ...
          <button
            onClick={() => {
              router.push(`${eventName}?tab=problems`);
            }}
            className={`flex items-center justify-center gap-2 sm:p-2 p-4 flex-1 text-xl ${
              activeTab === "problems"
                ? "bg-primary-light text-primary  fill-primary"
                : "fill-primary-light"
            } rounded-sm`}
          ...
          <button
            onClick={() => {
              router.push(`${eventName}?tab=participants`);
            }}
            className={`flex items-center justify-center gap-2 sm:p-2 p-4 flex-1 text-xl ${
  ...
  return (
    <div
      ...
      className={`flex flex-col ${
        activeQuestion === question._id ? "gap-4 bg-white/10 rounded" : "gap-0"
      } p-4 `}
    >
      <button
