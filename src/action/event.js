...
export const getUserEvents = async (userId) => {
    try {
        ...
        // })
        // return events;
        const contest = await Contest.find({ 'participants.user': userId });
        return contest.sort((a, b) => new Date(b.date) - new Date(a.date)).map(event => {
            return {
                name: event.name,
                date: event.date,
                rank: event.participants.find(participant => participant.user.toString() === userId.toString()).rank
            }
        });
