  ...
  return path === "/login" ? null : data?.user ? (
    <div className="relative w-10 h-10">
      ...
      {active && (
        <div
          ref={popUpRef}
          className="absolute -right-[1rem] top-[75px] w-96 bg-primary border border-white/10 z-[100] rounded-2xl flex flex-col gap-4 items-center p-4"
        >
          ...
          <div className="flex xs:flex-col gap-1 w-full">
            ...
            <button
              onClick={signOut}
              className="flex-1 bg-red-500 p-2 py-3 flex justify-center rounded-r-md xs:rounded-md hover:bg-red-600 transition-all items-center"
            >
              Logout
            </button>
          </div>
