...
const TeamParticipant = ({ team: t, contestId }) => {
    ...
    return (
        <div className="flex flex-col p-4">
            <button className={`flex gap-4 items-center transition-all ${expanded ? 'py-2' : ''}`} onClick={() => setexpanded(prev => !prev)}>
                ...
                <h2 className="font-bold text-2xl">{team[0].team}</h2>
                {
                    team[0].rank ?
                        <div className="ml-auto">
                            {team[0].rank <= 3 ? (
                                <Medal
                                    className={`h-12 w-12 -translate-y-0 sm:-translate-y-[23px] sm:w-10 sm:h-10 ${team[0].rank === 1
