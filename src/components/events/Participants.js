...
export const Participant = ({ participant: user, rank }) => {

    const participant = JSON.parse(user)

    return (
        <li className="">
            <Link href={`/user/${participant.username}`} className="flex items-center gap-6 sm:gap-4 p-4 border-b border-white/5 ">
                ...
                <div className="ml-auto">
                    {rank <= 3 ? (
                        <Medal
                            className={`h-14 w-14 -translate-y-4 sm:-translate-y-[23px] sm:w-10 sm:h-10 ${rank === 1
